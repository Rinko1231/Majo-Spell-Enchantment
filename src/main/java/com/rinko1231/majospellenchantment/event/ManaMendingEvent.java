...
import static com.rinko1231.majospellenchantment.init.ModEnchantments.MANA_MENDING_ENCHANT;
...
public class ManaMendingEvent {
    ...
    public void onManaChanged(ChangeManaEvent event) {
        if(MajoSpellEnchantmentConfig.manaMendingDisabled.get()) return;
        ...
        for (ItemStack stack : targets) {
            ...
            int level = getMendingLevel(stack);
            if (level <= 0) continue;
    ...
    private static int getMendingLevel(ItemStack stack) {
        return stack.getEnchantmentLevel(MANA_MENDING_ENCHANT.get());
    }
